...
/// Gluetun has its own icon in the service picker.
///
/// The picker loads `assets/service_icons/<kind>.png` and falls back to a
/// generic server icon when there is none, which Gluetun got until now.
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  test('the icon the picker asks for is bundled', () async {
    final ByteData icon = await rootBundle
        .load('assets/service_icons/${ServiceKind.gluetun.name}.png');
