...
class _ProwlarrIndexerFormScreenState
  ...
  final Map<String, bool> _obscure = <String, bool>{};
  ...
  Widget _buildField(Map<String, dynamic> field) {
    final ThemeData theme = Theme.of(context);
    final String type = (field['type'] as String?) ?? 'textbox';
    final String label =
      ...
      case 'checkbox':
        return Container(
          ...
          child: SwitchListTile(
            ...
            onChanged: (bool v) => setState(() => field['value'] = v),
          ),
        );
      case 'select':
        final List<Map<String, dynamic>> options =
            ((field['selectOptions'] as List<dynamic>?) ?? <dynamic>[])
                .map((dynamic e) => Map<String, dynamic>.from(e as Map<dynamic, dynamic>))
                .toList();
        final dynamic current = field['value'];
        final bool hasMatch =
        ...
        return Container(
          ...
          child: DropdownButtonFormField<dynamic>(
            ...
            items: options
                ...
                .toList(),
            onChanged: (dynamic v) => setState(() => field['value'] = v),
          ),
        );
      case 'number':
        return Container(
          ...
          child: TextFormField(
            ...
            onChanged: (String v) => field['value'] = num.tryParse(v) ?? v,
          ),
        );
      case 'password':
        final String name = (field['name'] as String?) ?? label;
        final bool obscure = _obscure[name] ?? true;
        return Container(
          margin: const EdgeInsets.only(bottom: Insets.md),
          child: TextFormField(
            initialValue: (field['value'] ?? '').toString(),
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                ...
                onPressed: () => setState(() => _obscure[name] = !obscure),
              ),
            ),
            onChanged: (String v) => field['value'] = v,
          ),
        );
      case 'textbox':
      default:
